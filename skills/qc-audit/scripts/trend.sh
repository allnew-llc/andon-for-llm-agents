...
#
# trend.sh - Quality self-assessment trend timeline (qc-audit canonical tool)
#
# Usage:
#   trend.sh [--help]
#
...
#   overall status, and criterion counts, prints a time-series table with
#   trend summary (improving / stable / degrading), per-phase ASCII bar
#   charts, and an aggregate rate summary.
...
Output:
  Prints a table of assessments sorted by date, per-phase bar charts,
  trend direction, and aggregate pass/warn/fail rate summary.
...
# ---------------------------------------------------------------------------
# Count occurrences of a quoted string value for "status" keys (awk gsub)
...
# ---------------------------------------------------------------------------
# Build ASCII bar for a single phase row (width=20 chars)
# Usage: ascii_bar <pass> <warn> <fail>
# ---------------------------------------------------------------------------
ascii_bar() {
    local pass_c="$1"
    local warn_c="$2"
    local fail_c="$3"
    local total=$((pass_c + warn_c + fail_c))
    local bar=""
    if [ "$total" -eq 0 ]; then
        echo "--------------------"
        return
    ...
    local bar_width=20
    local pass_w=$(( (pass_c * bar_width + total / 2) / total ))
    local warn_w=$(( (warn_c * bar_width + total / 2) / total ))
    local fail_w=$(( bar_width - pass_w - warn_w ))
    [ "$fail_w" -lt 0 ] && fail_w=0

    local i=0
    while [ "$i" -lt "$pass_w" ]; do bar="${bar}#"; i=$((i+1)); done
    i=0
    while [ "$i" -lt "$warn_w" ]; do bar="${bar}~"; i=$((i+1)); done
    i=0
    while [ "$i" -lt "$fail_w" ]; do bar="${bar}!"; i=$((i+1)); done
    # Pad to bar_width
    local blen=${#bar}
    while [ "$blen" -lt "$bar_width" ]; do bar="${bar}-"; blen=$((blen+1)); done
    echo "$bar"
}
...
# ---------------------------------------------------------------------------
# Trend detection: compare last 3 assessments
# ---------------------------------------------------------------------------
trend="stable"
...
# Rate Summary
# ---------------------------------------------------------------------------
total_criteria=$((total_pass + total_warn + total_fail))
echo ""
echo "Rate Summary:"
if [ "$total_criteria" -gt 0 ]; then
    pass_pct=$(( (total_pass * 100) / total_criteria ))
    warn_pct=$(( (total_warn * 100) / total_criteria ))
    fail_pct=$(( (total_fail * 100) / total_criteria ))
    printf '  PASS: %d%% (%d/%d criteria)\n' "$pass_pct" "$total_pass" "$total_criteria"
    printf '  WARN: %d%% (%d/%d criteria)\n' "$warn_pct" "$total_warn" "$total_criteria"
    printf '  FAIL: %d%% (%d/%d criteria)\n' "$fail_pct" "$total_fail" "$total_criteria"
else
    echo "  (no criteria data)"
fi

# ---------------------------------------------------------------------------
# Per-phase ASCII bar charts (# = pass, ~ = warn, ! = fail)
# ---------------------------------------------------------------------------
echo ""
echo "Phase Bar Charts  (# pass  ~ warn  ! fail)"
echo "------------------------------------------"
while IFS='|' read -r date_str phase_id overall pass_c warn_c fail_c; do
    [ -z "$date_str" ] && continue
    bar="$(ascii_bar "$pass_c" "$warn_c" "$fail_c")"
    printf '  %-15s [%s]\n' "$phase_id" "$bar"
done < "$sorted_file"
