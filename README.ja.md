...
コアランタイムに同梱。LLM コーディングエージェントの出力における無資格の専門業務を検知し、適切な免責事項を挿入します。
...
- 1 セッション内で異なるフェーズで 2 回以上 ANDON オープン
- 修正 → 実行 → 別の失敗 → 修正 → 実行 → 別の失敗（2 サイクル）

**対応：**
1. **停止** — 「とりあえず実行」を禁止
2. **パターン Five Whys** — 「なぜ毎回違うポイントで失敗するのか？」
3. **机上通し検証** — 実行前に残り全フェーズを机上でレビュー
4. **一括修正** — 1 つずつではなく、発見した問題をまとめて修正
...
特許通知：本ソフトウェアに実装されている特定の方法は、AllNew LLC が出願中の特許の対象です（特許出願中）。特許が付与された場合、Apache 2.0 第 3 条により、ユーザーには永続的かつロイヤリティフリーの特許ライセンスが供与されます。詳細は [NOTICE](./NOTICE) を参照してください。
