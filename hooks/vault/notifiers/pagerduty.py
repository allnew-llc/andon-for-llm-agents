...
_SEVERITY = {
    VaultEvent.SYNC_OK: "info",
    VaultEvent.SYNC_FAIL: "error",
    VaultEvent.ROTATE: "info",
    VaultEvent.ADD: "info",
    VaultEvent.REMOVE: "warning",
    VaultEvent.AUDIT_DRIFT: "critical",
    VaultEvent.AUDIT_OK: "info",
}
...
class PagerDutyNotifier(Notifier):
    ...
    def notify(self, payload: EventPayload) -> bool:
        ...
        if payload.event in (VaultEvent.SYNC_FAIL, VaultEvent.AUDIT_DRIFT):
            event_action = "trigger"
        elif payload.event in (VaultEvent.SYNC_OK, VaultEvent.AUDIT_OK):
