...
"""Local web UI for andon vault — serves on localhost only.

Launch with: andon vault ui
...
def _build_status_json(config: VaultConfig) -> list[dict]:
...
def _build_history_json(config: VaultConfig) -> dict[str, list[dict]]:
...
def _build_config_json(config: VaultConfig) -> dict:
...
class VaultUIHandler(BaseHTTPRequestHandler):
    """HTTP handler for the vault local UI."""
...
def serve(config: VaultConfig, port: int = DEFAULT_PORT) -> None:
    """Start the local web UI server."""
    handler = type("Handler", (VaultUIHandler,), {"config": config})

    server = HTTPServer(("127.0.0.1", port), handler)
    url = f"http://localhost:{port}"
    print(f"andon vault UI: {url}")
