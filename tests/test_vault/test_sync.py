...
"""Tests for vault.sync — sync orchestration."""
...
from vault.sync import check_status, remove_secret, sync_all, sync_secret


@pytest.fixture
def config() -> VaultConfig:
    cfg = VaultConfig(keychain_service="test-vault")
...
class TestSyncSecret:
    @patch("vault.sync.get_driver")
    @patch("vault.sync.keychain")
    def test_sync_success(self, mock_kc, mock_get_driver, config, tmp_path):
    ...
    @patch("vault.sync.keychain")
    def test_sync_keychain_missing(self, mock_kc, config, tmp_path):
    ...
    @patch("vault.sync.get_driver")
    @patch("vault.sync.keychain")
    def test_sync_driver_failure(self, mock_kc, mock_get_driver, config, tmp_path):
        mock_kc.get.return_value = "secret-val"
        mock_driver = MagicMock()
...
class TestSyncAll:
    @patch("vault.sync.get_driver")
    @patch("vault.sync.keychain")
    def test_sync_all(self, mock_kc, mock_get_driver, config, tmp_path):
...
class TestCheckStatus:
    @patch("vault.sync.get_driver")
    @patch("vault.sync.keychain")
    def test_check_status(self, mock_kc, mock_get_driver, config):
    ...
    @patch("vault.sync.get_driver")
    @patch("vault.sync.keychain")
    def test_check_status_missing(self, mock_kc, mock_get_driver, config):
        mock_kc.exists.return_value = True
        mock_driver = MagicMock()
...
class TestRemoveSecret:
    @patch("vault.sync.get_driver")
    @patch("vault.sync.keychain")
    def test_remove_success(self, mock_kc, mock_get_driver, config, tmp_path):
