...
"""Tests for vault.web — local web UI."""
...
from vault.web import VaultUIHandler, _build_config_json, _build_status_json, serve


@pytest.fixture
def sample_config() -> VaultConfig:
...
class TestBuildStatusJson:
    def test_returns_list(self, sample_config: VaultConfig):
        with patch("vault.web.check_status") as mock:
    ...
    def test_no_secret_values_in_output(self, sample_config: VaultConfig):
        with patch("vault.web.check_status") as mock:
            from vault.sync import StatusEntry
            mock.return_value = [
                StatusEntry(
...
class TestBuildConfigJson:
    def test_returns_metadata(self, sample_config: VaultConfig):
    ...
    def test_no_secret_values(self, sample_config: VaultConfig):
        result = _build_config_json(sample_config)
...
class TestServeIntegration:
    def test_html_page_served(self, sample_config: VaultConfig):
        """Test that the UI serves HTML on /."""
        port = 18390  # Use high port to avoid conflicts

        with patch("vault.web.webbrowser.open"):
    ...
    def test_api_config_endpoint(self, sample_config: VaultConfig):
    ...
    def test_404_for_unknown_path(self, sample_config: VaultConfig):
