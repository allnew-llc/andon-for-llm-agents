...
"""Tests for vault notification integrations."""
from __future__ import annotations

import subprocess
from unittest.mock import patch

from vault.config import NotifierConfig, VaultConfig
from vault.notifiers import NOTIFIER_MAP, get_notifier
from vault.notifiers.base import EventPayload, VaultEvent
from vault.notifiers.slack import SlackNotifier
from vault.notifiers.teams import TeamsNotifier
from vault.sync import SyncReport, SyncResult, fire_notifications
...
class TestSlackNotifier:
    @patch("vault.notifiers.slack.subprocess.run")
...
class TestTeamsNotifier:
    @patch("vault.notifiers.teams.subprocess.run")
    def test_notify_success(self, mock_run):
...
class TestFireNotifications:
    @patch("vault.notifiers.get_notifier")
    def test_fires_for_matching_events(self, mock_get):
        ...
        fire_notifications(config, VaultEvent.SYNC_FAIL, report, secret_name="x")
    ...
    def test_skips_non_matching_events(self, mock_get):
        ...
        fire_notifications(config, VaultEvent.SYNC_OK, report)
    ...
    def test_no_notifiers_configured(self):
        ...
        config = VaultConfig()
        report = SyncReport()
        fire_notifications(config, VaultEvent.SYNC_OK, report)  # no-op
...
class TestVaultConfigWithNotifiers:
